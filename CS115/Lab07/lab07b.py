...
# --- Our main function ---
def main():
    countries = ["Japan", "Bangladesh", "France", "Russia", "Sudan"]

    # asks the user for window width
    window_width = int(input("Window width: "))

    # if statement if window is within range
    if 100 <= window_width <= 1000:
        ...
        country_name = input("Name your countries: ")
        country_name = country_name.lower()
        country_list = country_name.split()
        for i in range(len(country_list)):
            if (country_list[i]) == "japan":
                draw_japan_flag(window_width)  # Draw a Japanese flag
            elif (country_list[i]) == "bangladesh":
                draw_bangladesh_flag(window_width)
            elif (country_list[i]) == "france":
                draw_france_flag(window_width)
            elif (country_list[i]) == "russia":
                draw_russia_flag(window_width)
            elif (country_list[i]) == "sudan":
                draw_sudan_flag(window_width)
            else:
                print(country_list[i], "is not a valid country")
    else:
